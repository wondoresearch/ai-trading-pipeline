...
class TestPITV2(unittest.TestCase):
    def history(self):
        return [
            FinancialHistory("BBRI", date(2021,12,31), date(2022,3,1), .40, "sa"),
            FinancialHistory("BBRI", date(2022,12,31), date(2023,3,1), .45, "sa"),
            FinancialHistory("BBRI", date(2023,12,31), date(2024,3,1), .50, "sa"),
            FinancialHistory("BBRI", date(2024,12,31), date(2025,3,1), .55, "sa"),
            FinancialHistory("BBRI", date(2025,12,31), date(2026,3,1), .60, "sa"),
        ]
    ...
    def test_future_publication_rejected(self):
        h=[FinancialHistory("PTBA",date(2025,12,31),date(2026,9,1),.9,"sa")]
    ...
    def test_cross_sector(self):
        for t in ("BBRI","PTBA","ADRO"):
            r=build_pit_state(t,date(2026,8,14),financial_history=[
                FinancialHistory(t,date(2025,12,31),date(2026,3,1),.7,"sa")
            ])
