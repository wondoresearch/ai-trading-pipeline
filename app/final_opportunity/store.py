    ...
    def __init__(self, path: Path):
        ...
        with sqlite3.connect(path) as db:
            ...
            if "available_at" not in cols:
                db.execute("ALTER TABLE prices ADD COLUMN available_at TEXT")
        ...
        with sqlite3.connect(self.path) as db:
            db.executemany(
                ...
                (ticker,trading_date,open,high,low,close,adj_close,volume,
                 source,provider_symbol,retrieved_at,available_at)
                VALUES(?,?,?,?,?,?,?,?,?,?,?,?)""",
                [
                    (
                        r["ticker"],
    ...
    def upsert_news(self, rows: Iterable[dict]) -> None:
        with sqlite3.connect(self.path) as db:
            db.executemany(
                ...
                (news_id,ticker,title,url,source,published_at,available_at,
                 sentiment,sentiment_label,sentiment_source)
                VALUES(?,?,?,?,?,?,?,?,?,?)""",
                [
                    (
                        r["news_id"],
    ...
    def prices(self, ticker: str, cutoff: str | None = None) -> list[dict]:
        with sqlite3.connect(self.path) as db:
            ...
            return [
                dict(x)
                for x in db.execute(
                    "SELECT * FROM prices WHERE ticker=? ORDER BY trading_date",
                    (ticker,),
                )
            ]
